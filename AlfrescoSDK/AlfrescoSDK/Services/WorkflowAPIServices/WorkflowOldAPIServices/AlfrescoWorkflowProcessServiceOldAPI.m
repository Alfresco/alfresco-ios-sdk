...
- (AlfrescoRequest *)retrieveProcessesInState:(NSString *)state completionBlock:(AlfrescoArrayCompletionBlock)completionBlock
{
    ...
    [self.session.networkProvider executeRequestWithURL:url session:self.session alfrescoRequest:request completionBlock:^(NSData *data, NSError *error) {
        ...
        else
        {
            ...
            NSArray *processes = [self workflowProcessesFromJSONData:data error:&conversionError];
...
- (AlfrescoRequest *)retrieveProcessWithIdentifier:(NSString *)processID completionBlock:(AlfrescoProcessCompletionBlock)completionBlock
{
    ...
    [self.session.networkProvider executeRequestWithURL:url session:self.session alfrescoRequest:request completionBlock:^(NSData *data, NSError *error) {
        ...
        {
            NSError *conversionError = nil;
            NSArray *tasks = [self workflowProcessesFromJSONData:data error:&conversionError];
...
- (AlfrescoRequest *)retrieveProcessImage:(AlfrescoWorkflowProcess *)process completionBlock:(AlfrescoContentFileCompletionBlock)completionBlock
{
    ...
    {
        NSString *requestString = [kAlfrescoWorkflowProcessImageOldAPI stringByReplacingOccurrencesOfString:kAlfrescoProcessID withString:process.identifier];
        NSURL *url = [AlfrescoURLUtils buildURLFromBaseURLString:self.baseApiUrl extensionURL:requestString];
...
- (AlfrescoRequest *)startProcessForProcessDefinition:(AlfrescoWorkflowProcessDefinition *)processDefinition assignees:(NSArray *)assignees variables:(NSDictionary *)variables attachments:(NSArray *)attachmentNodes completionBlock:(AlfrescoProcessCompletionBlock)completionBlock
{
    ...
    for (id nodeObject in attachmentNodes)
    {
        ...
        documentsAdded = [NSString stringWithFormat:@"%@,%@", documentsAdded, node.identifier];
    }
    
    if (documentsAdded)
    {
        [requestBody setValue:documentsAdded forKey:kAlfrescoOldBPMJSONProcessAttachments];
    }
    ...
    void (^parseAndSendCreationRequest)(AlfrescoRequest *request) = ^(AlfrescoRequest *request){
        ...
        NSString *completeProcessDefinitionIdentifier = [NSString stringWithFormat:@"%@%@", [AlfrescoWorkflowUtils prefixForActivitiEngineType:self.session.workflowInfo.workflowEngine], processDefinition.identifier];
...
//        request = [[AlfrescoRequest alloc] init];
        [self.session.networkProvider executeRequestWithURL:url session:self.session requestBody:requestData method:kAlfrescoHTTPPOST alfrescoRequest:request completionBlock:^(NSData *data, NSError *error) {
            ...
            {
                NSError *conversionError = nil;
                NSArray *workflowProcesses = [self workflowProcessesFromJSONData:data error:&conversionError];
                if (conversionError)
                {
...
- (AlfrescoRequest *)deleteProcess:(AlfrescoWorkflowProcess *)process completionBlock:(AlfrescoBOOLCompletionBlock)completionBlock
{
    ...
    NSString *requestString = [kAlfrescoWorkflowSingleProcessOldAPI stringByReplacingOccurrencesOfString:kAlfrescoProcessID withString:process.identifier];
    NSURL *url = [AlfrescoURLUtils buildURLFromBaseURLString:self.baseApiUrl extensionURL:requestString];
...
- (NSArray *)workflowProcessesFromJSONData:(NSData *)jsonData error:(NSError **)conversionError
{
    NSMutableArray *workflowProcesses = nil;
    ...
    workflowProcesses = [@[] mutableCopy];
    for (NSDictionary *entryDictionary in processArray)
    {
        [workflowProcesses addObject:[[AlfrescoWorkflowProcess alloc] initWithProperties:entryDictionary session:self.session]];
    }
