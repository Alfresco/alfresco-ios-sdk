...
#endif
unichar AlfrescoISO8601DefaultTimeSeparatorCharacter = DEFAULT_TIME_SEPARATOR;
...
@interface AlfrescoISO8601DateFormatter (UnparsingPrivate)
...
@implementation AlfrescoISO8601DateFormatter
- (id) init {
	if ((self = [super init])) {
		format = ISO8601DateFormatCalendar;
		timeSeparator = AlfrescoISO8601DefaultTimeSeparatorCharacter;
		includeTime = NO;
		parsesStrictly = NO;
	}
	return self;
...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
	...
	if (strict) timeSep = AlfrescoISO8601DefaultTimeSeparatorCharacter;
...
- (NSString *) weekDateStringForDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	...
	if(includeTime) {
		NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
		unichar timeSep = self.timeSeparator;
		if (!timeSep) timeSep = AlfrescoISO8601DefaultTimeSeparatorCharacter;
